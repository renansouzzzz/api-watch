...
namespace ApiWatch.Worker.Repositories;

public class WorkerEndpointRepository : IEndpointRepository
{
    private readonly AppDbContext _db;
    public WorkerEndpointRepository(AppDbContext db) => _db = db;
    ...
    {
        var ep = await GetByIdAsync(id, ct);
        if (ep is not null) { _db.MonitoredEndpoints.Remove(ep); await _db.SaveChangesAsync(ct); }
    }
}

public class WorkerCheckResultRepository : ICheckResultRepository
{
    private readonly AppDbContext _db;
    public WorkerCheckResultRepository(AppDbContext db) => _db = db;
